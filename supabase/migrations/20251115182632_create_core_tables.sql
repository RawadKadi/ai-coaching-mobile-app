/*
  # Core Database Schema for AI Coaching Platform

  ## Overview
  This migration creates the foundational tables for a multi-tenant coaching platform
  with AI-powered features, supporting coaches, clients, and admins.
  ...
  ### `coach_client_links`
  Many-to-many relationship between coaches and clients
  - `id` (uuid, PK)
  ...
  ### `ai_coach_brains`
  AI configuration per coach (custom coaching personality/rules)
  - `id` (uuid, PK)
  - `coach_id` (uuid, FK to coaches)
  - `tone` (text)
  - `style` (text)
  ...
  - Coaches: coaches can manage their own data
  - Clients: clients can read their own data
  - Coach_client_links: coaches see their clients, clients see their coaches
  - AI_coach_brains: only the coach can access their brain config

  ## 3. Indexes
  - Profile lookups by user_id
  - Coach/client lookups
...
-- AI Coach Brains (AI configuration per coach)
...
-- RLS Policies for coach_client_links
...
CREATE POLICY "Clients can view their coach links"
  ON coach_client_links FOR SELECT
  TO authenticated
  USING (
    EXISTS (
