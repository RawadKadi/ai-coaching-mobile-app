...
-- =====================================================
-- Compatible with your schema structure
...
BEGIN
    ...
    -- Check authorization (coach or client via their IDs)
    SELECT (
...
BEGIN
    -- Trigger 1: Missed sessions (high priority)
    RETURN QUERY
    SELECT DISTINCT
        c.id AS client_id,
        coach.id AS coach_id,
        'missed_sessions' AS trigger_type,
        jsonb_build_object(
            'sessions_missed', COUNT(s.id),
            'last_session', MAX(s.scheduled_at)
        ) AS trigger_data,
        5 AS priority
    FROM clients c
    ...
    GROUP BY c.id, coach.id
    HAVING COUNT(s.id) >= 2; -- 2 or more cancelled sessions
    
    -- Trigger 2: Low meal logging (medium priority)
    RETURN QUERY
    SELECT DISTINCT
        c.id AS client_id,
        coach.id AS coach_id,
        'low_meal_logging' AS trigger_type,
        jsonb_build_object(
            'meals_logged', COUNT(m.id),
            'days_checked', 7
        ) AS trigger_data,
        4 AS priority
    FROM clients c
    JOIN coach_client_links ccl ON ccl.client_id = c.id
    JOIN coaches coach ON coach.id = ccl.coach_id
    LEFT JOIN meals m ON m.client_id = c.id
    ...
    GROUP BY c.id, coach.id
    HAVING COUNT(m.id) < 5; -- Less than 5 meals in 7 days
    
    -- Trigger 3: No recent workouts (medium priority)
    RETURN QUERY
    SELECT DISTINCT
        c.id AS client_id,
        coach.id AS coach_id,
        'no_recent_workouts' AS trigger_type,
        jsonb_build_object(
            'workouts_completed', COUNT(w.id),
            'days_checked', 7
        ) AS trigger_data,
        3 AS priority
    FROM clients c
    ...
    GROUP BY c.id, coach.id
    HAVING COUNT(w.id) = 0; -- Zero workouts in 7 days
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
DO $$ 
BEGIN 
    RAISE NOTICE '✅ RPC functions migration completed successfully!'; 
    RAISE NOTICE '   - Created 10 functions for challenge management';
