const fs = require('fs');
const file = 'lib/ai-scheduling-service.ts';
let content = fs.readFileSync(file, 'utf8');

const target = `export const extractSchedulingIntent = async (input: string): Promise<IntentExtractionResponse> => {
    try {
        const prompt = \`
        ...
        for (const line of lines) {
            ...
            if (key === 'sessions') {
                ...
                for (const part of parts) {
                    ...
                    const sessionTime = (timeStr && timeStr !== 'null') ? timeStr : null;
...
const replacement = `export const extractSchedulingIntent = async (input: string): Promise<IntentExtractionResponse> => {
    try {
        const prompt = \`
Extract scheduling intent from this coach input. 
